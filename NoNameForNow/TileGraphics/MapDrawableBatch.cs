...
namespace FlatRedBall.TileGraphics
{
    public class MapDrawableBatch : PositionedObject, IDrawableBatch
    {
		...
		public void RemoveQuads(IEnumerable<int> quadIndexes)
        {
            ...
            // Now let's loop through all the dictionaries and remove the indexes as necessary:
            ...
            foreach (var kvp in mNamedTileOrderedIndexes)
            {
                ...
                for (int i = 0; i < ints.Count; i++)
                {
                    ...
                    if (lastPassedIndex == ints.Count)
                    {
                        ints[i] -= lastPassedIndex;
                    }
                    else if (ints[i] == orderedInts[lastPassedIndex])
                    {
                        ints.Clear();
                        break;
                    }
                    ...
                    {
                        while (lastPassedIndex < orderedInts.Count && ints[i] > orderedInts[lastPassedIndex])
                        {
                            lastPassedIndex++;
                        }
                        if (lastPassedIndex < orderedInts.Count && ints[i] == orderedInts[lastPassedIndex])
                        {
                            ints.Clear();
                            break;
                        }
